...
.page-content h1, .page-content .hero-title {
  font-size: 1.875rem;
  font-weight: 700;
  color: #1e3a8a;
...
export default function ClientPage({ params, initialData, initialError }) {
  ...
  const sanitizeHtml = (html) => {
    // Remove class="hero-title" from <h1> while preserving other attributes
    const modifiedHtml = html.replace(/<h1\b([^>]*?)\bclass="hero-title"([^>]*)>/gi, "<h1$1$2>");
    const sanitized = DOMPurify.sanitize(modifiedHtml, {
      ALLOWED_TAGS: [
        "section",
        "article",
        "div",
      ...
      ALLOWED_ATTR: ["class", "id", "href", "src", "alt", "style", "data-sheets-root"],
    });
    console.log("[Client] Sanitized HTML:", sanitized); // Debug to check if <h1> is preserved
    return sanitized;
  };
  ...
  // Check if sanitized HTML contains an <h1> tag
  const sanitizedContent = pageData?.content ? sanitizeHtml(pageData.content) : "";
  const hasH1 = /<h1\b[^>]*>/i.test(sanitizedContent);

  return (
    <div>
      <Head>
        <title>{pageData?.title || "Coffee Packaging Solutions | Nexibles"}</title>
        <meta
          name="description"
          content={
            pageData?.metadescription ||
            "Discover Nexibles' coffee packaging solutions with high-barrier films, degassing valves, and eco-friendly options."
          }
        />
        <meta
          name="keywords"
          content={
            pageData?.keywords ||
            "coffee packaging, high-barrier films, degassing valves, sustainable coffee packaging, Nexibles"
          }
        />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name="canonical" content={`https://www.nexibles.com/industries/${params.slug}`} />
        <meta property="og:title" content={pageData?.title || "Coffee Packaging Solutions | Nexibles"} />
        <meta
        ...
        <meta
          name="twitter:title"
          content={pageData?.title || "Coffee Packaging Solutions | Nexibles"}
        />
        <meta
        ...
        <div className="mx-auto mt-16 page-content font-poppins">
          ...
          {hasH1 ? (
            ...
            <div className="font-poppins">
              <h1>{pageData?.title || "Coffee Packaging Solutions | Nexibles"}</h1>
