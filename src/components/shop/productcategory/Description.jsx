...
const DescriptionSection = ({ description, certifications = [] }) => {
  ...
  const renderContent = (tabData) => {
    switch (tabData.type) {
      case 'paragraph':
        return <p className="text-gray-600 leading-7">{tabData.content}</p>;
      case 'list':
        return (
          <ul className="space-y-2">
            {tabData.content.map((item, index) => (
              <li key={index} className="flex items-start">
                <span className="w-2 h-2 bg-[#ffd13e] rounded-full mt-2 mr-3 flex-shrink-0"></span>
            ...
            {tabData.content.map((faq, index) => (
              ...
              >
                <button
                  ...
                  className="w-full flex justify-between items-center px-4 py-3 text-left font-medium text-gray-800 hover:bg-gray-50 focus:outline-none transition-colors duration-200"
                >
                ...
                {openFaqIndex === index && (
                  <div className="px-4 pb-3 text-gray-600 transition-all duration-300">
                    {faq.answer}
                  </div>
                )}
              </div>
      ...
      case 'message':
        return <p className="text-gray-500 italic text-center">{tabData.content}</p>;
      default:
        return <p className="text-gray-600">{tabData.content}</p>;
    }
  };

  return (
    <div className="bg-gray-50 rounded-xl p-8 max-w-6xl mx-auto shadow-sm transition-all duration-300">
      {/* Tab Navigation */}
      <div className="flex border-b border-gray-200 mb-8 space-x-2">
        {tabs.map((tab) => (
          <button
            ...
            className={`relative px-4 py-3 text-sm font-semibold rounded-t-lg transition-all duration-300 ${
              activeTab === tab.id
          ...
          >
            {tab.label}
            {activeTab === tab.id && (
              <span className="absolute bottom-0 left-0 w-full h-1 bg-[#ffd13e] rounded-t-lg"></span>
            )}
          </button>
        ))}
      </div>
      ...
      {/* Tab Content */}
      <div className="text-base leading-relaxed mb-8 transition-opacity duration-300">
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="flex flex-wrap gap-3">
            {certifications.map((cert, index) => (
              <div
                key={index}
                className="flex items-center space-x-2 bg-white rounded-full px-4 py-2 text-sm text-gray-700 shadow-sm hover:shadow-md transition-shadow duration-200"
              >
