...
export async function generateMetadata({ params }) {
  // Extract the category slug (first segment of the path)
  const category = params.categories.split('/')[0]; // Ensure only the first segment is used (e.g., 'spout-pouch')
  const content = contentData.categories?.[category];

  if (!content) {
    return {
      title: 'Category Not Found | Nexibles',
      description: 'The requested product category was not found. Explore our range of packaging solutions at Nexibles.',
      robots: 'noindex',
    };
  }
  ...
  return {
    ...
    keywords: content.topKeywords.map(k => k.split(' (')[0]).join(', '),
    robots: 'index, follow',
    viewport: 'width=device-width, initial-scale=1.0',
    alternates: {
      canonical: content.url, // Uses the full URL from JSON (e.g., https://www.nexibles.com/spout-pouch/custom-liquid-gel-spout-pouches)
    },
    openGraph: {
      title: content.metaTitle,
      description: content.metaDescription,
      ...
      images: [
        {
          url: content.hero.images.main.src,
    ...
    twitter: {
      ...
      title: content.metaTitle,
      description: content.metaDescription,
      images: [content.hero.images.main.src],
    },
  };
}
...
export default function ProductCategory({ params }) {
  ...
  if (!content) {
    return (
      <main className="bg-white text-black min-h-screen flex flex-col items-center justify-center">
        <Head>
          <link rel="icon" href="/favicon.ico" type="image/x-icon" />
          <link rel="icon" href="/favicon.png" type="image/png" />
        </Head>
        <h1 className="text-4xl font-bold text-center mt-16">Category Not Found</h1>
        <p className="text-red-500 text-lg mb-4 text-center">The requested category was not found.</p>
        <a
          href="/"
          className="block text-center bg-yellow-400 text-black font-bold py-2 px-4 rounded-full hover:scale-105 transition-transform duration-300 mx-auto max-w-xs"
        >
          Go to Home
        </a>
      </main>
  ...
  return (
    <>
      ...
      <ProductCategoryClient content={content} category={category} />
    </>
  );
}
